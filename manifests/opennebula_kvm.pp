...
class opennebula::node::kvm($ensure="present") inherits opennebula::node::common{ 
    notice("${hostname} should be ${pres_real}")
    
    $pkgs = $lsbdistid ? {
        "Ubuntu" => ["kvm", "qemu-kvm-extras", "kvm-pxe","libvirt-bin", "qemu-kvm" ],
        default => [ "libvirt-bin", "qemu-kvm", "qemu-user","qemu-utils","etherboot-qemu" ]
    }
    package{$pkgs:
    ensure => $ensure,
    }
...
#remove strange private network config
  ...
  file{"/etc/udev/rules.d/45-kvm.rules":
    content => "KERNEL=='kvm', GROUP='${opennebula::node::common::libvirtgroup}', OWNER='oneadmin', MODE='0660'\n",
      notify => Exec["opennebula-reload-udev"],
  }
  exec{"opennebula-reload-udev":
